...
void palRunThreads()
{
  ...
  status = pal_osThreadCreate(palThreadFunc3, &g_threadsArg, PAL_osPriorityNormal, 1024, stack3, NULL, &threadID3);
  TEST_ASSERT_EQUAL(PAL_SUCCESS, status); 

  status = pal_osThreadCreate(palThreadFunc4, &g_threadsArg, PAL_osPriorityBelowNormal, 1024, stack4, NULL, &threadID4);
  TEST_ASSERT_EQUAL(PAL_SUCCESS, status); 

  status = pal_osThreadCreate(palThreadFunc5, &g_threadsArg, PAL_osPriorityAboveNormal, 1024, stack5, NULL, &threadID5);
  TEST_ASSERT_EQUAL(PAL_SUCCESS, status); 

  status = pal_osThreadCreate(palThreadFunc6, &g_threadsArg, PAL_osPriorityHigh, 1024, stack6, NULL, &threadID6);
  TEST_ASSERT_EQUAL(PAL_SUCCESS, status); 
