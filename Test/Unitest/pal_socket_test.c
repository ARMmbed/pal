...
TEST(pal_socket, basicSocketScenario4)
{
	...
	result = pal_getAddressInfo("www.yahoo.com", &address, &addlen);
	TEST_ASSERT_EQUAL(result, PAL_SUCCESS);
...
TEST(pal_socket, basicSocketScenario5)
{
	...
	TEST_PRINTF("waiting for connection2:\r\n");
	result = pal_accept(sock, &address2, &addrlen, &sock3);
	TEST_PRINTF("after accept:\r\n");
	TEST_ASSERT_EQUAL(result, PAL_SUCCESS);


	result = pal_recv(sock3, buffer, 99, &read);
	TEST_ASSERT_EQUAL(result, PAL_SUCCESS);
	TEST_PRINTF(buffer);

	result = pal_send(sock3, messageOut, 15, &sent);
	TEST_ASSERT_EQUAL(result, PAL_SUCCESS);
